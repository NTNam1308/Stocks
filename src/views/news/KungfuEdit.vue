        ...
        <CCardHeader>
          <CForm>
            ...
            <CRow class="btn-profile">
              ...
              <CCol col="6" xs="6" sm="6" md="6" lg="2" class="btn-edit-center">
                <CButton color="primary" id="editNew" @click="updateNew()">Lưu</CButton>
              </CCol>
              <CCol  lg="4"/>   
...
export default {
  ...
  methods: {
    ...
    updateNew() {
        let self = this;
        axios.post(  this.$apiAdress + '/api/kungfu-news/' + self.$route.params.id + '?token=' + localStorage.getItem("api_token"),
        {
            _method: 'PUT',
            mack: self.mack,
            title: self.title,
        ...
        .then(function (response) {
            ...
            self.message = 'Cập nhật tin tức thành công.';
            setTimeout(function () {
          ...
          self.showMessageError = true;
            if(error.response){
              ...
              if( error.response.data.errors.mack ){
                  ...
                  self.messageError = 'Cập nhật tin tức thất bại.';
              }
  ...
  mounted: function(){
    ...
    let self = this;
     window.addEventListener('keyup', event => {
      if (event.keyCode === 13) { 
        if(self.title != '' ){
          document.getElementById("editNew").click();
        }
      }
    });
    axios.get(  this.$apiAdress + '/api/kungfu-news/' + self.$route.params.id + '/edit?token=' + localStorage.getItem("api_token"))
    .then(function (response) {
        self.mack = response.data.mack;
        self.title = response.data.title;
        self.content = response.data.content;
        self.category_id = response.data.category_id;
        self.url = response.data.thumbnail;
        self.thumbnail = response.data.checkThumbnail;
    }).catch(function (error) {
        console.log(error);

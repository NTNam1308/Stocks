...
export default {
  ...
  data() {
    return {
      ...
      data_color: [
        "rgb(229, 48, 42)",
        "rgba(229, 48, 42, 0.937)",
        "rgba(229, 48, 42, 0.514)",
        "rgba(229, 48, 42, 0.447)",
        "rgba(229, 48, 42, 0.255)",
        "rgba(229, 48, 42, 0.224)",
        "rgba(229, 48, 42, 0.208)",
        "rgba(229, 48, 42, 0.173)",
        "rgba(229, 48, 42, 0.173)",
        "rgba(229, 48, 42, 0.1)",
      ],
  ...
  computed: {
    item_table() {
      ...
      if (this.data_item.length > 0) {
        let temp_data = this.data_item;
        temp_data.sort((a, b) => parseFloat(a.perchange) - parseFloat(b.perchange));
        return_data = temp_data.map((item, index) => {
  ...
  watch: {
    data_filter_type: {
      handler() {
        this.$emit("updatedTypeDateTopDown", this.data_filter_type);
      },
    },
    data_filter_type_board: {
      handler() {
        this.$emit("updatedTypeBoardTopDown", this.data_filter_type_board);
      },
    },
  },
};
